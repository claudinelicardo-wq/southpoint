...
function RetailDialog({
  ...
  const [form, setForm] = useState({
    ...
    reorder_level: linkedItem?.reorder_level ?? 0,
    purchase_unit_label: linkedItem?.purchase_unit_label ?? "",
    purchase_to_base_factor: linkedItem?.purchase_to_base_factor ?? 1,
  ...
  async function save(e: React.FormEvent) {
    ...
    if (product) {
      const { error } = await supabase
        .from("products")
        .update({ name: form.name, category_id: form.category_id, price: form.price })
        .eq("id", product.id);
      if (error) {
      ...
      if (!itemId) {
        const { data, error } = await supabase
          ...
          .insert({
            ...
            purchase_to_base_factor: form.purchase_to_base_factor || 1,
            reorder_level: form.reorder_level,
            track_expiry: form.track_expiry,
          })
          .select("id")
